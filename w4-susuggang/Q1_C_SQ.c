...
void createQueueFromLinkedList(LinkedList *ll, Queue *q)
{
    ...
    // 연결 리스트의 모든 노드를 큐에 삽입합니다.
    while (cur != NULL) {
        enqueue(q, cur->item);  // 큐에 값을 삽입합니다.
...
void removeOddValues(Queue *q)
{
    int size = q->ll.size;  // 큐의 원래 크기 저장
    int item;

    while (size > 0) {      // 큐의 원래 크기만큼 반복
        item = dequeue(q);  // 큐에서 값을 하나 꺼냅니다.
        size--;             // 남은 아이템 수 감소
...
void removeAllItemsFromQueue(Queue *q)
{
	...
	for (i = 0; i < count; i++) // 선입선출 원리에 따라, i = 0 부터(첫 원소)부터 dequeue 진행
		dequeue(q);
}
