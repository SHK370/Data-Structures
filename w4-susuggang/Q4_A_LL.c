...
void moveEvenItemsToBack(LinkedList *ll)
{
    ...
    // 리스트 순회 시작
    while (cur != NULL) {
        ListNode *nextNode = cur->next; // 현재 노드의 다음 노드를 미리 저장
		...
		// 그냥 cur = cur->next; 로 적어도 됨
    }

    // 리스트의 끝 처리
    if (evenTail != NULL) {
        evenTail->next = NULL; // 짝수 리스트의 마지막 노드는 NULL로 설정
    }

    if (oddTail != NULL) {
        oddTail->next = evenHead; // 홀수 리스트 뒤에 짝수 리스트 연결
        ll->head = oddHead; // head를 홀수 리스트의 첫 번째 노드로 설정
    } else {
        ll->head = evenHead; // 홀수 리스트가 없으면 짝수 리스트가 head가 됨
    }
}
