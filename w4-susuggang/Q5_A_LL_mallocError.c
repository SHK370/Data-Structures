...
void frontBackSplitLinkedList(LinkedList *ll, LinkedList *resultFrontList, LinkedList *resultBackList)
{
    ...
    // 초기화: resultFrontList와 resultBackList를 빈 리스트로 초기화
    removeAllItems(resultFrontList);
    removeAllItems(resultBackList);
    ...
    // 두 개의 포인터 (slow와 fast)를 사용하여 리스트를 절반으로 나누기
    // slow는 한 칸씩 이동, fast는 두 칸씩 이동
    while (fast != NULL && fast->next != NULL) {
        prev = slow;            // 이전 slow 위치 저장
        slow = slow->next;      // slow는 한 칸씩 이동
        fast = fast->next->next; // fast는 두 칸씩 이동
    }

    // 여기까지 오면 slow는 중간 위치를 가리키고, prev는 그 앞을 가리킴
    if (prev != NULL) {
        prev->next = NULL; // 앞부분 리스트와 뒷부분 리스트를 분리
    }

    // 앞쪽 절반은 ll의 head부터 slow 이전까지
    resultFrontList->head = ll->head;
    resultFrontList->size = (ll->size + 1) / 2; // 홀수일 경우 앞 리스트가 더 길게 설정
