...
void recursiveReverse(Queue *q) {
    ...
    int temp = dequeue(q);         // Dequeue the front element
    recursiveReverse(q);           // Recursively reverse the remaining queue
    enqueue(q, temp);              // Enqueue the front element at the rear
}
