...
int balanced(char *expression)
{
    ...
    for (int i = 0; expression[i] != '\0'; i++) {
        ...
        if (current == '(' || current == '[' || current == '{') {
            ...
            // If closing bracket, check if stack is empty or top of stack matches
            if (isEmptyStack(&s)) {
                return 0;  // Unbalanced because no matching opening bracket
            }
            ...
            if ((current == ')' && top == '(') ||
                (current == ']' && top == '[') ||
                (current == '}' && top == '{')) {
                pop(&s);  // Pop matching opening bracket
            } else {
                return 0;  // Unbalanced because of mismatch
            }
        }
    }

    // After processing all characters, the stack should be empty if balanced
